...
//readonly attribute HTMLCanvasElement canvas;
  ...
  var not_implemented = function(name) {
    return function() {
      console.log("NOT_IMPLEMENTED:", name)
      fail();
      return 1;
    }
  };
  ...
  var that = this;
  that.notImplemented = 0;
  that.totalMethods = methods.length;
  methods.forEach(function(method) {
    var func = that[method];
    if (!func) {
      that.notImplemented++;
      func = function() {
...
WebGLRenderingContext.prototype = {
  ...
  * General                  *
  ***************************/
  clear : function clear (bits) {
    webgl_rendering_context_clear(bits);
  },
  flush : function flush () {
    webgl_rendering_context_flush();
  },
  clearColor : function clearColor (r, g, b, a) {
    webgl_rendering_context_clearColor(r, g, b, a);
  },
  getShaderParameter : function getShaderParameter (shader, pname) {
    return webgl_rendering_context_getShaderParameter(shader.id, pname);
  },
  enable : webgl_rendering_context_enable,
  blendFunc: webgl_rendering_context_blendFunc,
  ...
  ***************************/
  ...
  attachShader : function attachShader (program, shader) {
    webgl_rendering_context_attachShader(program.id, shader.id);
  },
  linkProgram : function linkProgram (program) {
    webgl_rendering_context_linkProgram(program.id);
  },
  useProgram : function useProgram (program) {
    program.use();
  },
  getProgramParameter : function getProgramParameter (program, param) {
    return program.getParameter(param);
  },
  getProgramInfoLog : function getProgramInfoLog (program) {
    return program.getInfoLog();
  },
  getAttribLocation : function getAttribLocation (program, attr) {
  ...
  ***************************/
  ...
  bindTexture : function bindTexture (target, texture) {
    ...
    webgl_rendering_context_bindTexture(target, texture.id);
  },
  pixelStorei : function pixelStorei (param, value) {
    webgl_rendering_context_pixelStorei(param, value);
  },
  texImage2D : function texImage2D (target, level, internalFormat, width, height, border, format, type, pixels) {
    // handle images
    if (typeof border === 'object' && border.data) {
      // remap the args
      format = width;
      type = height;
      var image = border;

      webgl_rendering_context_texImage2D_Image(target, level, internalFormat, format, type, image);
    }
  },
  activeTexture : function activeTexture (texture_id) {
    webgl_rendering_context_activeTexture(texture_id);
  },
  ...
  ***************************/
  viewport : function viewport (x, y, w, h) {
    webgl_rendering_context_viewport(x, y, w, h);
  }

};

function WebGLProgram() {
  this.id = webgl_rendering_context_createProgram();
}
WebGLProgram.prototype = {
  use : function use () {
    webgl_rendering_context_useProgram(this.id);
  },
  getParameter : function getParameter (param) {
    return webgl_rendering_context_getProgramParameter(this.id, param);
  },
  getInfoLog : function getInfoLog () {
    return webgl_rendering_context_getProgramInfoLog(this.id);
  },
  getAttribLocation : function getAttribLocation (attr) {
    return webgl_rendering_context_getAttribLocation(this.id, attr);
  }
};

function WebGLShader(type) {
  this.type = type;
  this.id = webgl_rendering_context_createShader(type);
}

WebGLShader.prototype = {
  source : function source (str) {
    webgl_rendering_context_shaderSource(this.id, str);
  },
  compile : function compile () {
    webgl_rendering_context_compileShader(this.id);
  },
  getInfoLog : function getInfoLog () {
    return webgl_rendering_context_getShaderInfoLog(this.id);
  }
};

function WebGLTexture() {
  this.id = webgl_rendering_context_createTexture();
}

WebGLTexture.prototype = {};

function WebGLRenderBuffer() {
  this.id = webgl_rendering_context_createRenderBuffer();
}

WebGLRenderBuffer.prototype = {};

function WebGLBuffer() {
  this.id = webgl_rendering_context_createBuffer();
}
WebGLBuffer.prototype = {
  bind : function bind (target) {
    webgl_rendering_context_bindBuffer(target, this.id);
  }
};
...
DOMImage.prototype = {
  data : null,
  set src(url) {
    var loaded = readimage(url);
    this.data = loaded.data;
    this.width = loaded.width;
    this.height = loaded.height;
    this.bytesPerPixel = loaded.bytesPerPixel;
